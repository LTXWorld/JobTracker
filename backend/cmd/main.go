...
func main() {
	...
	// 初始化服务
	jobRepo := repository.NewJobApplicationRepository(db)
	jobService := service.NewJobApplicationService(jobRepo)
	authRepo := repository.NewAuthRepository(db)
	authService := service.NewAuthService(authRepo)
	statusConfigRepo := repository.NewStatusConfigRepository(db)
	statusConfigService := service.NewStatusConfigService(statusConfigRepo)
	statusTrackingRepo := repository.NewStatusTrackingRepository(db)
	statusTrackingService := service.NewStatusTrackingService(statusTrackingRepo, statusConfigRepo)
	...
	// 监控相关路由
	api.HandleFunc("/monitoring/db-stats", databaseStatsHandler.GetDatabaseStats).Methods("GET")
	api.HandleFunc("/monitoring/connection-stats", databaseStatsHandler.GetConnectionPoolStats).Methods("GET")
	api.HandleFunc("/monitoring/reset-stats", databaseStatsHandler.ResetPerformanceStats).Methods("POST")
