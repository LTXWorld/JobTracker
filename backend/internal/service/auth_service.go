...
func (s *AuthService) ChangePassword(userID uint, req *model.ChangePasswordRequest) error {
	...
	// 更新密码
	if err := s.repo.UpdatePassword(userID, hashedPassword, time.Now()); err != nil {
...
func (s *AuthService) UpdateAvatar(userID uint, file multipart.File, header *multipart.FileHeader) (string, int, error) {
	...
	currentVersion, err := s.repo.GetAvatarVersion(userID)
	if err != nil {
		return "", 0, fmt.Errorf("failed to query avatar version: %w", err)
	}
	...
	// 更新数据库
	if err := s.repo.UpdateAvatar(userID, relPath, newVersion, time.Now()); err != nil {
...
func (s *AuthService) createUser(username, email, hashedPassword string) (*model.User, error) {
	return s.repo.CreateUser(username, email, hashedPassword, time.Now())
}
...
func (s *AuthService) IsUsernameAvailable(username string) (bool, error) {
	exists, err := s.repo.UsernameExistsInsensitive(username)
	if err != nil {
...
func (s *AuthService) IsEmailAvailable(email string) (bool, error) {
	exists, err := s.repo.EmailExistsInsensitive(email)
	if err != nil {
		return false, fmt.Errorf("database query failed: %w", err)
	}
	return !exists, nil
