...
func NewStatusConfigRepository(db *database.DB) StatusConfigRepository {
	return &statusConfigRepo{db: db}
}
...
func (r *statusConfigRepo) GetFlowTemplates(userID uint) ([]model.StatusFlowTemplate, error) {
	...
	for rows.Next() {
		...
		if createdBy.Valid {
			v := uint(createdBy.Int64)
			t.CreatedBy = &v
		}
		if len(cfg) > 0 {
			var m map[string]interface{}
			_ = jsonUnmarshal(cfg, &m)
			t.FlowConfig = m
		}
		list = append(list, t)
	}
...
func (r *statusConfigRepo) CheckTemplateNameExists(name string, excludeID *int) (bool, error) {
	...
	defer cancel()
	if excludeID != nil {
		if err := r.db.ORM.WithContext(ctx).Raw("SELECT EXISTS(SELECT 1 FROM status_flow_templates WHERE name=$1 AND id<>$2 AND is_active=true)", name, *excludeID).Row().Scan(&exists); err != nil {
			return false, err
		}
	} else {
		if err := r.db.ORM.WithContext(ctx).Raw("SELECT EXISTS(SELECT 1 FROM status_flow_templates WHERE name=$1 AND is_active=true)", name).Row().Scan(&exists); err != nil {
			return false, err
		}
	}
	return exists, nil
...
func (r *statusConfigRepo) CreateFlowTemplate(userID uint, name string, desc *string, flowConfigBytes []byte) (*model.StatusFlowTemplate, error) {
	...
	var t model.StatusFlowTemplate
	if err := r.db.ORM.WithContext(ctx).Raw(q, name, desc, flowConfigBytes, userID).Row().Scan(&t.ID, &t.CreatedAt, &t.UpdatedAt); err != nil {
		return nil, err
	}
	t.Name = name
	t.Description = desc
...
func (r *statusConfigRepo) UpdateFlowTemplate(userID uint, templateID int, name string, desc *string, flowConfigBytes []byte) (*model.StatusFlowTemplate, error) {
	...
	if err := row.Scan(&t.ID, &t.Name, &d, &cfg, &t.IsDefault, &t.IsActive, &createdBy, &t.CreatedAt, &t.UpdatedAt); err != nil {
		return nil, err
	}
	if d.Valid {
...
func (r *statusConfigRepo) GetDefaultFlowTemplate() (int, string, error) {
	...
	if err := r.db.ORM.WithContext(ctx).Raw(q).Row().Scan(&id, &text); err != nil {
		return 0, "", err
	}
	return id, text, nil
}
...
func (r *statusConfigRepo) GetPreferences(userID uint) (*model.UserStatusPreferences, error) {
	...
	if err := row.Scan(&pref.ID, &pref.UserID, &cfg, &pref.CreatedAt, &pref.UpdatedAt); err != nil {
		return nil, err
	}
	if len(cfg) > 0 {
		var m map[string]interface{}
		_ = jsonUnmarshal(cfg, &m)
		pref.PreferenceConfig = m
	}
	return &pref, nil
}
...
func (r *statusConfigRepo) UpsertPreferences(userID uint, preferenceBytes []byte, now time.Time) (*model.UserStatusPreferences, error) {
	...
	if err := row.Scan(&pref.ID, &pref.UserID, &out, &pref.CreatedAt, &pref.UpdatedAt); err != nil {
		return nil, err
	}
	if len(out) > 0 {
		var m map[string]interface{}
		_ = jsonUnmarshal(out, &m)
		pref.PreferenceConfig = m
	}
	return &pref, nil
}
