...
### 阶段三：新增功能组件 ✅
1. **密码强度与规则提示**：
   - 在注册页 `Register.vue` 内集成密码强度条
   - 新增实时规则提示：是否包含"至少一个大写字母"和"至少一个特殊字符"
   - 文案随输入即时切换"已包含/需包含"，帮助用户一次性通过校验

2. **服务条款复选框修复**：
   - 修复注册页面同意服务条款复选框无法点击的问题
   - 将 agreement 字段集成到表单数据中
   - 确保表单验证与数据绑定的一致性

3. **网络状态指示器组件** (`NetworkStatus.vue`):
   - 全局网络连接状态监控
   - 断线/重连时的顶部通知栏
   - 3秒自动隐藏恢复提示
   - 智能的显示时机控制
...
### 新增/现有组件
- `/src/components/PasswordStrength.vue` - 密码强度指示器（可复用）
- `/src/components/NetworkStatus.vue` - 网络状态监控

### 优化文件
- `/src/api/auth.ts` - API路径修正，健康检查优化
- `/src/api/request.ts` - 网络监控机制增强
- `/src/views/auth/Register.vue` - 修复服务条款复选框交互与勾选动画；新增密码规则提示
- `/src/views/auth/Login.vue` - 登录安全锁定机制
- `/src/views/auth/Profile.vue` - 修复头像上传功能的TypeScript类型错误
